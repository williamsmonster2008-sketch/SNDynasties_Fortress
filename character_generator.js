...
export class CharacterGenerator {
  ...
  async generateRandomCharacter(options = {}) {
    ...
    try {
      ...
      console.log(`👤 随机角色生成完成: ${character.name} (${character.gender}, ${character.age}岁)`);
      return character;
  ...
  async buildCharacterConfig(options = {}) {
    ...
    if (options.name) {
      ...
      try {
        ...
        console.warn('⚠️ NameGenerator 调用失败，使用备用方案:', error.message);
    ...
    const config = {
      ...
      familyName: options.familyName || nameResult.surname,
  ...
  _generateBackupName(gender, socialClass) {
    const backupSurnames = ['李', '王', '张', '刘', '陈', '杨', '赵', '黄', '周', '吴'];
    const maleNames = ['明', '华', '强', '军', '伟', '峰', '勇', '涛', '磊', '鹏'];
    const femaleNames = ['丽', '娟', '芳', '静', '敏', '霞', '燕', '红', '梅', '玲'];
    
    const surname = Utils.Array.randomChoice(backupSurnames);
    ...
    return {
      ...
      surname: surname,
      givenName: givenName,
      nameInfo: { meaning: '备用姓名', category: 'fallback' }
    };
  }
  ...
  async generateFamilyMembers(familyConfig = {}) {
    console.log(`👨‍👩‍👧‍👦 生成家族成员: ${familyConfig.familyName}`);
