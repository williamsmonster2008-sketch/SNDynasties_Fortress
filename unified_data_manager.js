...
export class UnifiedDataManager {
  ...
  async createCharacter(config) {
    console.log('🆕 开始创建角色:', config.name || 'Unknown');
  ...
  async validateCharacterConfig(config) {
    ...
    const errors = [];
    // 检查必需字段
    if (!config.name) errors.push('缺少name字段');
    ...
    // 基础验证
    const validation = this.validator.validate('character', config);
