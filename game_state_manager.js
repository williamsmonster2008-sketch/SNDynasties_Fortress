...
export class GameStateManager {
  ...
  syncCharacterStates() {
    ...
    const newPopulation = {
      ...
      alive: characters.filter(c => c.isAlive !== false && c.health > 0).length,
      male: characters.filter(c => c.gender === '男').length,
      female: characters.filter(c => c.gender === '女').length,
      children: characters.filter(c => c.age < 16).length,
      adults: characters.filter(c => c.age >= 16 && c.age < 60).length,
      elders: characters.filter(c => c.age >= 60).length
    };
   ...
   */
  syncFamilyStates() {
    if (!this.gameEngine.families) return;
    
    const families = Array.from(this.gameEngine.families.values());
    const newFamilies = {
      total: families.length,
      familyList: families.map(f => ({
        name: f.familySurname || f.familyName,
        memberCount: f.totalMembers || 0,
        type: f.familyType || 'unknown'
      })),
      averageSize: families.length > 0 ? 
        families.reduce((sum, f) => sum + (f.totalMembers || 0), 0) / families.length : 0,
      prosperityLevels: {}
    };
    
    // 统计繁荣等级
    families.forEach(family => {
      const level = family.prosperityLevel || 'unknown';
      newFamilies.prosperityLevels[level] = (newFamilies.prosperityLevels[level] || 0) + 1;
    });
  ...
  syncRelationshipStates() {
    if (!this.gameEngine.globalRelationships) return;
    ...
    this.gameEngine.globalRelationships.forEach(relationshipSystem => {
      const relationships = relationshipSystem.getAllRelationships?.() || [];
      totalRelationships += relationships.length;
      
      relationships.forEach(rel => {
        if (rel.primaryType === 'family') {
          familyBonds++;
    ...
    const maxPossibleRelationships = totalCharacters * (totalCharacters - 1) / 2;
