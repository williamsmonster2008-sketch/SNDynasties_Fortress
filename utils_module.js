...
export const ObjectUtils = {
  ...
  deepDiff(obj1, obj2) {
    ...
    for (const key of allKeys) {
      ...
      if (!(key in obj1)) {
        diff[key] = { type: 'added', newValue: val2 };
      } else if (!(key in obj2)) {
        diff[key] = { type: 'removed', oldValue: val1 };
      } else if (!this.deepEqual(val1, val2)) {
        if (typeof val1 === 'object' && typeof val2 === 'object' && val1 !== null && val2 !== null) {
          const nestedDiff = this.deepDiff(val1, val2);
          if (Object.keys(nestedDiff).length > 0) {
            diff[key] = { type: 'nested', diff: nestedDiff };
          }
        } else {
          diff[key] = { type: 'changed', oldValue: val1, newValue: val2 };
        }
      }
    }
