...
export class EmotionalRelationshipSystem {
  ...
  getAllRelationships(filters = {}) {
    console.log('调用getAllRelationships，bloodRelations存在:', !!this.bloodRelations);
     // 从角色对象获取血缘关系
    const character = window.gameEngine?.characters?.get(this.characterId);
    console.log('角色对象检查:', {
       hasCharacter: !!character,
       characterId: this.characterId,
       hasBloodRelations: !!character?.bloodRelations,
       bloodRelationsSize: character?.bloodRelations?.size || 0
    });
    let relationships = Array.from(this.relationships.values());
        
    if (character && character.bloodRelations) {
      console.log('从角色对象获取血缘关系，数量:', character.bloodRelations.size);
      character.bloodRelations.forEach((relation, targetId) => {
        relationships.push({
          targetId: targetId,
          type: 'blood',
          displayName: relation.relation,
          isActive: true,
          getEmotionalStrength: () => 60
        });
      });
    }
