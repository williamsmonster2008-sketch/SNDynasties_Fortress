...
export class GameInterface {
  ...
  updateCharactersDisplay(characters) {
    ...
    if (this.gameEngine && this.gameEngine.characters) {
      ...
      console.log('📦 直接从游戏引擎获取角色，数量:', allCharacters.length);
      
      // 显示所有存活角色，不限制血缘关系
    characterArray = allCharacters.filter(char => {
      // 基本存活检查
      const isAlive = char.vitalStatus === 'living';
      
      // 确保角色有基本属性
      const hasBasicData = char.name && char.age !== undefined;
      
      return isAlive && hasBasicData;
    });

    console.log(`🔍 过滤结果: 总角色${allCharacters.length}个，存活角色${characterArray.length}个`);
      // 验证德行系统
