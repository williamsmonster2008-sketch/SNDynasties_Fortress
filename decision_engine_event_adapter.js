...
export function addEventSupportToDecisionEngine(decisionEngine, eventBus) {
  ...
  decisionEngine.checkDecisionPatterns = function(characterId, decision, eventBus) {
    ...
    // 检查风险偏好变化
    decisionEngine.checkRiskPreferenceShift(characterId, recentDecisions, eventBus);
  };

  // 添加风险偏好变化检查方法
  decisionEngine.checkRiskPreferenceShift = function(characterId, recentDecisions, eventBus) {
    if (!recentDecisions || recentDecisions.length < 5) return;
      
    // 分析最近决策的风险水平
    const riskLevels = recentDecisions.map(decision => 
        decision.factors?.riskLevel || 0.5
    );
      
    // 计算早期和后期风险偏好
    const earlyRisk = riskLevels.slice(0, Math.floor(riskLevels.length / 2))
        .reduce((sum, risk) => sum + risk, 0) / Math.floor(riskLevels.length / 2);
      
    const recentRisk = riskLevels.slice(Math.floor(riskLevels.length / 2))
        .reduce((sum, risk) => sum + risk, 0) / Math.ceil(riskLevels.length / 2);
      
    const riskShift = recentRisk - earlyRisk;
      
    // 如果风险偏好发生显著变化，发出事件
    if (Math.abs(riskShift) > 0.3) {
        eventBus.emit('riskPreferenceShift', {
            characterId,
            shift: riskShift > 0 ? 'more_risky' : 'more_conservative',
            magnitude: Math.abs(riskShift),
            earlyAverage: earlyRisk,
            recentAverage: recentRisk,
            timestamp: Date.now()
        });
    }
  };
