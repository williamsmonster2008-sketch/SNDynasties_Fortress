...
export class FamilyNetworkService {
  ...
  async _addSpousesForGeneration(structure, members, generation, fertilityCfg, socialClass,unitId, idManager) {
    console.log(`【家族${unitId}】addspouseForGeneration方法开始，接收到的members引用: ${members}`);
    console.log(`【家族${unitId}】方法开始，members.size: ${members.size}`);
    
    // 先看看所有成员的情况
    const allMembers = Array.from(members.values());
    console.log(`所有第${generation}代成员:`, allMembers.filter(m => m.generation === generation));
    // 获取本代所有单身成员
    ...
    const thisGenSingles = Array.from(members.values()).filter(member => {
      ...
      //const isLiving = member.vitalStatus === 'living';

      console.log(`成员筛选 - ID:${member.characterId}, gen:${member.generation}, age:${member.age}, canBreed:${member.canBreed}, status:${member.vitalStatus}`);
      console.log(`筛选结果 - 世代匹配:${isRightGeneration}, 可育:${canBreed}, 年龄合格:${hasValidAge}`);

      return isRightGeneration && canBreed && hasValidAge;
    });
  
    console.log(`🎯 第${generation}代筛选出的单身成员数: ${thisGenSingles.length}`);
    thisGenSingles.forEach(member => {
      console.log(`- ID:${member.characterId}, 性别:${member.gender}, 年龄:${member.age}`);
      if (!member.age || member.age < 13) {
        console.error(`❌ 异常：成员${member.characterId}年龄过小或未分配: ${member.age}岁`);
      }
    });
    ...
    // 在结婚率计算后添加
    console.log(`结婚率配置:`, marriageConfig);
    console.log(`随机结婚率: ${marriageRate}`);
    console.log(`选中结婚成员: ${selectedToMarry.length} / ${thisGenSingles.length}`);    
    
    // 为选中成员创建婚姻并立即生成配偶详细信息
    for (const member of selectedToMarry) {      
      const wifeId = member.gender === '女' ? member.characterId : 
        idManager.allocateId(`spouse_wife_${generation}`);
      const husbandId = member.gender === '男' ? member.characterId :
        idManager.allocateId(`spouse_husband_${generation}`);
      ...
      if (member.canBreed === false ) {
        console.log(`成员 ${member.characterId} 无法生育，跳过配偶生成`);
        continue; // 跳过这个成员
      }
      ...
      if (member.gender === '男') {
        ...
        const minMarriageAge = this.fertilityConfig.min_breeding_age;
        const maxMarriageAge = this.fertilityConfig.max_breeding_age.female;
        const coupleAgeDiff = fertilityCfg.couple_age_difference;
        const ageDiff = Utils.Math.randomInt(coupleAgeDiff.min, coupleAgeDiff.max);
        const rawWifeAge = Math.random() < 0.6 ? 
          member.age - ageDiff : member.age + ageDiff;
        const wifeAge = Math.max(minMarriageAge, Math.min(maxMarriageAge, rawWifeAge));
        ...
        members.set(wifeId, {
          ...
          characterId: wifeId,
          canBreed: true,
          originalFamily: await this._generateSpouseFamilyName(socialClass, excludeSurname),
          currentFamily: member.currentFamily || 'unknown',
          socialClass: this._determineSpouseSocialClass(socialClass)
        });
        console.log(`✅ 为男性 ${member.characterId} 创建了妻子 ${wifeId}`);
        console.log(`妻子是否成功添加到members: ${members.has(wifeId)}`);
        ...
        if (marriagePattern === 'matrilocal') {
          ...
          const excludeSurname = member.familyName || member.originalFamily;
          const minMarriageAge = this.fertilityConfig.min_breeding_age;
          const maxMarriageAge = this.fertilityConfig.max_breeding_age.male;
          const coupleAgeDiff = fertilityCfg.couple_age_difference;
          const ageDiff = Utils.Math.randomInt(coupleAgeDiff.min, coupleAgeDiff.max);
          const rawHusbandAge = Math.random() < 0.4 ? 
            member.age - ageDiff : member.age + ageDiff;          
          const husbandAge = Math.max(minMarriageAge, Math.min(maxMarriageAge, rawHusbandAge));
          ...
          members.set(husbandId, {
            ...
            adoptedIntoFamily: member.currentFamily
          });
        } else {
          // 嫁出模式
          member.vitalStatus = 'married_away';
    ...
    console.log(`第${generation}代最终婚配关系:`);
    structure.marriages.filter(m => m.generation === generation).forEach((marriage, index) => {
      const husband = members.get(marriage.husband);
      const wife = members.get(marriage.wife);
      console.log(`婚姻${index + 1}: 夫: ${husband?.characterId || 'null'}(${husband?.gender || 'none'}), 妻: ${wife?.characterId || 'null'}(${wife?.gender || 'none'})`);
      console.log(`  - 夫存在: ${!!husband}, 妻存在: ${!!wife}`);
      console.log(`  - 夫状态: ${husband?.vitalStatus || 'undefined'}, 妻状态: ${wife?.vitalStatus || 'undefined'}`);
      console.log(`  - 夫canBreed: ${husband?.canBreed}, 妻canBreed: ${wife?.canBreed}`);
    });     
    // 添加成员统计（在方法结束的 } 之前）
    if (generation === 2) {
      const gen2Members = Array.from(members.values()).filter(m => m.generation === 2);
      console.log(`🔍 第2代总成员数: ${gen2Members.length}`);
      gen2Members.forEach(member => {
        console.log(`  - ${member.characterId}: ${member.gender}, 年龄${member.age}, 状态${member.vitalStatus}`);
      });      
    }       
  }
  ...
  /**
   * 基于当前代实际婚姻生成下一代子女关系
   * @param {Object} structure - 家族结构
   * @param {Map} members - 成员数据Map
   * @param {number} currentGen - 当前代数
   * @param {Object} idManager - ID管理器
   */
  ...
  _updateMarriageStatus(male, female, relation) {
    ...
    try {
      ...
      console.log(`✅ 婚姻状态和归属更新完成: ${male.name} × ${female.name}`);
